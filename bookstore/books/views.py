...
class MyProfileView(TemplateView):
    ...
    def get_queryset(self):
        return MyProfileView.objects.filter(is_visible=True)
    
class MyProfileView(ListView):
    model = Profile
    template_name = 'books/my_profile.html' 
    content_object_name = 'profiles' 
    def get_queryset(self):
        return Profile.objects.filter(email ='chotukha930@gmail.com')
    
    
class AddProfileView(TemplateView):
    template_name = 'books/profileform.html'
    def get_context_data(self, **kwargs):

        context = super().get_context_data(**kwargs)
        context['form'] = AddProfileView()
        return context
